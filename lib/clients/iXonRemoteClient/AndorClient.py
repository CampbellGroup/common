...
class AndorClient(QWidget):
    ...
    def connect(self):
        ...
        self.cxn = yield connectAsync(
            self.cameraIP, name=self.name, password=self.password
        )
        ...
        yield self.server.addListener(
            listener=self.image_updated, source=None, ID=IMAGE_UPDATED_SIGNAL
        )
    ...
    def on_live_button(self, checked):
        if checked:
            ...
            self.binx, self.biny, self.startx, self.stopx, self.starty, self.stopy = (
                yield self.server.get_image_region(None)
            )
...
class image_region_selection_dialog(QDialog):
    ...
    def do_submit(self, bin_hor, bin_ver, start_hor, stop_hor, start_ver, stop_ver):
        if self.parent.live_update_loop.running:
            ...
            try:
                yield self.server.get_image_region(
