    ...
    def full_parameter(self):
        if self._units:
            ...
            return (
                "parameter",
                [
                    WithUnit(self._min, self._units),
                    WithUnit(self._max, self._units),
...
class ScanNode(Node):
        ...
        if self._units:
            return (
                ...
                (
                    ...
                    (
                        WithUnit(self._scan_start, self._units),
                        WithUnit(self._scan_stop, self._units),
                        self._scan_points,
                    ),
                ),
            )
    ...
    def string_format(self):
        if self._units:
            return "Scan {0} {3} to {1} {3} in {2} steps".format(
                self._scan_start, self._scan_stop, self._scan_points, self._units
            )
        else:
            return "Scan {0} to {1} in {2} steps".format(
                self._scan_start, self._scan_stop, self._scan_points
            )
...
class SidebandElectorNode(Node):
    ...
    def full_parameter(self):
        return (
            "sideband_selection",
            [self._radial1, self._radial2, self._axial, self._micromotion],
        )
...
class SpectrumSensitivityNode(Node):
    ...
    def full_parameter(self):
        return (
            ...
            (
                self.WithUnit(self._span, "kHz"),
                self.WithUnit(self._resolution, "kHz"),
                self.WithUnit(self._duration, "us"),
                self.WithUnit(self._amplitude, "dBm"),
            ),
        )
