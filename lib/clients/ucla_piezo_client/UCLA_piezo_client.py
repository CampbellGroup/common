...
class ucla_piezo_client(QWidget):
    ...
    def initializeGUI(self):
        ...
        for i in range(4):
            ...
            chan_button.TTLswitch.toggled.connect(
                lambda state=chan_button.TTLswitch.isDown(), chan=i + 1: self.on_chan_toggled(
                    state, chan
                )
            )
            voltage_spin_box = QDoubleSpinBox()
            ...
            voltage_spin_box.valueChanged.connect(
                lambda volt=voltage_spin_box.value(), chan=i + 1: self.voltage_changed(
                    volt, chan
                )
            )
