...
class MultiplexerServer(LabradServer):
    ...
    def set_pid_sensitivity(self, c, dacPort, sensitivityFactor, sensitivityExponent):
        ...
        yield self.wmdll.SetPIDSetting(
            self.DeviationUnit, port_c, self.UseFrequencyUnits, self.d
        )

        yield self.wmdll.SetPIDSetting(
            self.DeviationSensitivityDimension, port_c, sExponent, self.d
        )

        yield self.wmdll.SetPIDSetting(
            self.DeviationSensitivityFactor, port_c, self.l, sFactor
        )
    ...
    def set_pid_polarity(self, c, dacPort, polarity):
        ...
        if polarity == 1 or polarity == -1:
            ...
            yield self.wmdll.SetPIDSetting(
                self.DeviationPolarity, port_c, value, self.d
            )

        else:
            returnValue("Polarity must be +/- 1")

    @setting(42, "set_channel_lock", dacPort="w", waveMeterChannel="w", lock="b")
    def set_channel_lock(self, c, dacPort, waveMeterChannel, lock):
        ...
        if course <= 0:
            ...
            if lock == 1:
                yield self.wmdll.SetPIDSetting(
                    self.DeviationChannel, port_c, chan_c, self.d
                )

            elif lock == 0:
                yield self.wmdll.SetPIDSetting(
                    self.DeviationChannel, port_c, ctypes.c_long(0), self.d
                )
    ...
    def get_channel_lock(self, c, dacPort, waveMeterChannel):
        ...
        yield self.wmdll.GetPIDSetting(
            self.DeviationChannel, port_c, ctypes.pointer(wmChannel), self.d
        )
    ...
    def get_const_dt(self, c, dacPort):
        ...
        yield self.wmdll.GetPIDSetting(
            self.PIDConstdt, port_c, ctypes.pointer(dt), dummyarg
        )
    ...
    def get_pid_sensitivity(self, c, dacPort):
        ...
        yield self.wmdll.GetPIDSetting(
            self.DeviationSensitivityDimension,
            port_c,
            ctypes.pointer(sExponent),
            dummyarg,
        )

        yield self.wmdll.GetPIDSetting(
            self.DeviationSensitivityFactor, port_c, self.l, ctypes.pointer(sFactor)
        )
    ...
    def get_pid_polarity(self, c, dacPort):
        ...
        yield self.wmdll.GetPIDSetting(
            self.DeviationPolarity, port_c, ctypes.pointer(polarity), dummyarg
        )
    ...
    def get_wavemeter_pattern(self, c, chan):
        ...
        """
        yield self.wmdll.GetPatternDataNum(
            ctypes.c_ulong(chan), ctypes.c_long(0), self.pattern1_ptr
        )
