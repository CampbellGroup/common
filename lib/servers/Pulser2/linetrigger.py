...
class LineTrigger(LabradServer):
    ...
    def line_trigger_state(self, c, enable=None):
        if enable is not None:
            if enable:
                yield self.inCommunication.run(self._enable_line_trigger, self.linetrigger_duration)
            else:
                yield self.inCommunication.run(self._disable_line_trigger)
    ...
    def line_trigger_duration(self, c, duration=None):
        ...
        if duration is not None:
            if self.linetrigger_enabled:
                yield self.inCommunication.run(self._enable_line_trigger, duration)
