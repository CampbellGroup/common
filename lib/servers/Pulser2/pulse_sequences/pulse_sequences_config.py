...
class dds_channel(object):
    ...
    def freq_conversion(self, freq):
        """
        can be overwritten to provide a custom frequency conversion
        """
        return freq

    def ampl_conversion(self, ampl):
        """
        can be overwritten to provide a custom amplitude conversion
        """
        return ampl

    def phase_conversion(self, phase):
        """
        can be overwritten to provide a custom phase conversion
        """
        return phase
...
class double_pass_729(dds_channel):
    ...
    def phase_conversion(self, phase):
        ...
        phase = (
            -phase
        )  # flip the phase such that DDS follows sin(w t - phi), see writeup on single qubit operations
