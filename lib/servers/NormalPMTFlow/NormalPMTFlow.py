...
class NormalPMTFlow(LabradServer):
    ...
    def setup_listeners(self):
        ...
        yield self.client.manager.addListener(listener=self.follow_server_connect, source=None, ID=9898989)
        yield self.client.manager.addListener(listener=self.follow_server_disconnect, source=None, ID=9898989 + 1)
    ...
    def set_time_length(self, c, timelength):
        ...
        if not self.collect_time_range[0] <= timelength['s'] <= self.collect_time_range[1]:
    ...
    def _record(self):
        ...
        if len(rawdata) != 0:
            if self.current_mode == 'Normal':
                ...
                to_data_vault = [[elem[2] - self.startTime, elem[0], 0, 0] for elem in rawdata]
            ...
            self.process_requests(to_data_vault)  # if we have any requests, process them
    ...
    def convert_differential(self, rawdata):
        ...
        for dataPoint in rawdata:
            ...
            total_data.append([dataPoint[2] - self.startTime,
                               self.last_differential['ON'],
                               self.last_differential['OFF'], diff])
        return total_data
