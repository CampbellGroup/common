...
class UtilityMethodsTest(_TestCase):
    ...
    def test_labrad_urlencode(self):
        ...
        expected = "data:application/labrad;base64," "AAAAAXMAAAAPAAAAC2Zvby5iYXIvYmF6"
        self.assertEqual(expected, actual)

    def test_labrad_urldecode(self):
        url_string = (
            "data:application/labrad;base64," "AAAAAXMAAAAPAAAAC2Zvby5iYXIvYmF6"
        )
    ...
    def test_labrad_urldecode_incorrect_prefix(self):
        ...
        self.assertRaises(ValueError, backend.labrad_urldecode, url_string)
...
class SelfClosingFileTest(_TestCase):
    ...
    def setUp(self):
        ...
        self.file = backend.SelfClosingFile(
            opener=self.opener,
    ...
    def test_file_opens_on_init(self):
        ...
        self.assertEqual(self.opener.args, ("1", "2", "3"), "File args not set")
        self.assertEqual(
    ...
    def test_closes_file_after_timeout(self):
        ...
        self.file.on_close(onCloseCallback)
        # Advance clock to close the self closing file.
        self.clock.advance(self.close_timeout_sec)
        self.assertFalse(self.opener.file.is_open, msg="File not closed after timeout")
        self.assertTrue(
            self.close_callback_called, msg="Registered callback not called!"
        )


# Dependent and Independent variables used for testing IniData and HDF5MetaData.
_INDEPENDENTS = [
    backend.Independent(label="FirstVariable", shape=(1,), datatype="v", unit="Ghz"),
    backend.Independent(
        label="SecondVariable", shape=(1,), datatype="v", unit="Kelvin"
    ),
]

_DEPENDENTS = [
    backend.Dependent(
        label="Cents", legend="OnlyDependent", shape=(1,), datatype="v", unit="Dollars"
    )
]
...
class _MetadataTest(_TestCase):
    ...
    def test_initialize_info(self):
        ...
        self.assertEqual(data.getTransposeType(), "(*v[Ghz],*v[Kelvin],*v[Dollars])")
        self.assertEqual(data.getDependents(), _DEPENDENTS)
        self.assertEqual(data.getIndependents(), _INDEPENDENTS)
    ...
    def test_add_param(self):
        ...
        param = (True, np.int32(100), U.Complex(1.0j + 0xDEADBEEF, U.inch))
        data.addParam("Param1", param)
        self.assertEqual(data.getParamNames(), ["Param1"])
        self.assertEqual(data.getParameter("Param1"), param)
        self.assertRaises(errors.BadParameterError, data.getParameter, "param1")
        self.assertEqual(data.getParameter("param1", case_sensitive=False), param)
    ...
    def test_add_param_already_added(self):
        ...
        param = (True, np.int32(100), U.Complex(1.0j + 0xDEADBEEF, U.inch))
        data.addParam("Param1", param)
        self.assertRaises(errors.ParameterInUseError, data.addParam, "Param1", param)
...
"""
    ...
    def test_load_dtype(self):
        ...
        self.assertEqual(
            data.dtype, np.dtype([("f0", "<f8"), ("f1", "<f8"), ("f2", "<f8")])
        )

    def test_load_independents(self):
        data = self.load_test_data()
        independents = data.get_independents()
    ...
    def test_load_parameters(self):
        data = self.load_test_data()
        parameters = data.get_param_names()
        self.assertEqual(len(parameters), 1)
        self.assertEqual(parameters[0], "A Parameter")
        self.assertEqual(data.get_parameter("A Parameter"), [12, 3, {"a": 0}])

    def test_load_comments(self):
        data = self.load_test_data()
        self.assertEqual(data.num_comments(), 2)
        comments, next_comment_position = data.get_comments(None, 0)
        self.assertEqual(next_comment_position, 2)
        self.assertEqual(len(comments), 2)
        self.assertEqual(
            comments[0], (datetime.datetime(2012, 9, 24, 3, 14, 15), "bar", "baz")
        )
        self.assertEqual(
            comments[1], (datetime.datetime(2012, 9, 25, 3, 14, 15), "fizz", "buzz")
        )

    def test_load_rowtype(self):
        data = self.load_test_data()
        self.assertEqual(data.get_row_type(), "*(v[GHz],v[Kelvin],v[Dollars])")

    def test_load_transpose_type(self):
        data = self.load_test_data()
        self.assertEqual(data.get_transpose_type(), "(*v[GHz],*v[Kelvin],*v[Dollars])")

    def test_add_complicated_param(self):
        data = backend.IniData()
        data.initialize_info("FooTitle", _INDEPENDENTS, _DEPENDENTS)
        self.assertEqual(data.get_param_names(), [])
        data.add_param("Param1", ("really", {"complex": 0xDEADBEEF}, ["data"]))
        self.assertEqual(data.get_param_names(), ["Param1"])
        self.assertEqual(
            data.get_parameter("Param1"), ("really", {"complex": 0xDEADBEEF}, ["data"])
        )
        self.assertRaises(errors.BadParameterError, data.get_parameter, "param1")
        self.assertEqual(
            data.get_parameter("param1", case_sensitive=False),
    ...
    def test_save_reload(self):
        ...
        data_to_save.initialize_info("FooTitle", _INDEPENDENTS, _DEPENDENTS)
        data_to_save.add_comment("foo user", "bar comment")
        data_to_save.add_param("Param1", [100])
        # Save it.
        ...
        self.assertEqual(data.get_dependents(), _DEPENDENTS)
        self.assertEqual(data.get_independents(), _INDEPENDENTS)
        comments, _ = data.get_comments(1, 0)
        self.assertEqual(len(comments), 1)
        self.assertEqual(comments[0][1], "foo user")
        self.assertEqual(comments[0][2], "bar comment")
        self.assertEqual(data.get_param_names(), ["Param1"])
        self.assertEqual(data.get_parameter("Param1"), [100])
        self.assertEqual(data.get_row_type(), "*(v[Ghz],v[Kelvin],v[Dollars])")
        self.assertEqual(data.get_transpose_type(), "(*v[Ghz],*v[Kelvin],*v[Dollars])")
...
class _BackendDataTestCase(_TestCase):
    def assert_data_in_backend(self, backend_data, expected_data):
        ...
        read_data, next_pos = backend_data.get_data(None, 0, False, None)
        self.assert_arrays_equal(read_data, expected_data)
        self.assertEqual(next_pos, len(expected_data))

        # Read using getData for first row of data.
        read_data, next_pos = backend_data.get_data(1, 0, False, None)
        self.assert_arrays_equal(read_data[0], expected_data[0])
        self.assertEqual(next_pos, 1)

        # Read using getData for first row of data.
        read_data, next_pos = backend_data.get_data(1, 1, False, None)
...
class CsvListDataTest(_BackendDataTestCase):
    ...
    def test_add_data_then_read(self):
        self.data.add_data([[1, 2, 3]])
        self.data.add_data([[4, 5, 6]])
        self.assert_data_in_backend(self.data, [[1, 2, 3], [4, 5, 6]])

    def test_add_data_wrong_number_of_columns(self):
        self.assertRaises(errors.BadDataError, self.data.add_data, [(1, 2)])
        self.assertRaises(errors.BadDataError, self.data.add_data, [(1, 2, 3, 4)])

    def test_read_from_file(self):
        # Add some data and save it to a file.
        self.data.add_data([[1, 2, 3], [4, 5, 6]])
        self.data.save()
        del self.data
        ...
        self.assertTrue(data.has_more(0))
        self.assertTrue(data.has_more(1))
        self.assertFalse(data.has_more(2))
...
class _BackendDataTest(_BackendDataTestCase):
    ...
    def test_add_data_then_read(self):
        ...
        data1[0] = (4, 5, 6)
        self.data.add_data(data0)
        self.data.add_data(data1)
        self.assert_data_in_backend(self.data, [[1, 2, 3], [4, 5, 6]])

    def test_add_recarray_data_then_read(self):
        data = np.recarray((2,), dtype=[("f0", "<f8"), ("f1", "<f8"), ("f2", "<f8")])
        data[0] = (1, 2, 3)
        data[1] = (4, 5, 6)
    ...
    def test_read_from_file(self):
        ...
        data_to_save = np.recarray(
            (2,), dtype=[("f0", "<f8"), ("f1", "<f8"), ("f2", "<f8")]
        )
        data_to_save[0] = (1, 2, 3)
        data_to_save[1] = (4, 5, 6)
        self.data.add_data(data_to_save)
        self.data.save()
        del self.data
    ...
    def test_get_data_transpose(self):
        ...
        data_to_add[0] = (1, 2, 3)
        data_to_add[1] = (4, 5, 6)
        self.data.add_data(data_to_add)

        self.assertRaises(RuntimeError, self.data.get_data, None, 0, True, None)
...
class ExtendedHDF5DataTest(_BackendDataTest):
    ...
    def test_empty_data_read(self):
        read_data, _ = self.data.get_data(None, 0, False, None)
        self.assertEqual(read_data, [])
    ...
    def test_get_data_transpose(self):
        ...
        actual, next_pos = self.data.get_data(None, 0, True, None)
        self.assertEqual(next_pos, 2)
    ...
    def test_initialize_info_bad_vars(self):
        bad_independents = [
            backend.Independent(
                label="FirstVariable", shape=(1,), datatype="f", unit="Ghz"
            )
        ]
        data = self.get_backend_data(self.filename)
        self.assertRaises(
            RuntimeError, data.initialize_info, "FooTitle", bad_independents, []
        )
        ...
        self.assertRaises(
            RuntimeError, data.initialize_info, "FooTitle", bad_dependents, []
        )
    ...
    def test_initialize_type_i(self):
        ...
        independent = backend.Independent(
            label="NewVariable", shape=(1,), datatype="i", unit=""
        )
    ...
    def test_initialize_type_t(self):
        ...
        data = self.get_backend_data(name)
        independent = backend.Independent(
            label="NewVariable", shape=(1,), datatype="t", unit=""
        )
        data.initialize_info("Foo", [independent], [])
    ...
    def test_initialize_type_c(self):
        ...
        data = self.get_backend_data(name)
        independent = backend.Independent(
            label="NewVariable", shape=(1,), datatype="c", unit=""
        )
        data.initialize_info("Foo", [independent], [])
    ...
    def test_initialize_type_v(self):
        ...
        data = self.get_backend_data(name)
        independent = backend.Independent(
            label="NewVariable", shape=(1,), datatype="v", unit=""
        )
        data.initialize_info("Foo", [independent], [])
    ...
    def test_initialize_type_v(self):
        ...
        data = self.get_backend_data(name)
        independent = backend.Independent(
            label="NewVariable", shape=(1,), datatype="s", unit=""
        )
        data.initialize_info("Foo", [independent], [])
    ...
    def test_initialize_type_unknown(self):
        ...
        data = self.get_backend_data(name)
        independent = backend.Independent(
            label="NewVariable", shape=(1,), datatype="x", unit=""
        )
        self.assertRaises(
            RuntimeError, data.initialize_info, "FooTitle", [independent], []
        )
    ...
    def test_add_complex_data_array_then_read(self):
        ...
        data = self.get_backend_data(name)
        independent = backend.Independent(
            label="NewVariable", shape=(2, 2), datatype="c", unit="V"
        )
        data.initialize_info("Foo", [independent], [])
        data_entry = np.recarray((1,), dtype=[("f0", "<c16", (2, 2))])
        data_entry[0][0][0] = [0j, 1j]
        data_entry[0][0][1] = [1j, 0j]
        data.add_data(data_entry)
        added_data, _ = data.get_data(None, 0, False, None)
    ...
    def test_add_string_array_column(self):
        ...
        data = self.get_backend_data(name)
        independent = backend.Independent(
            label="NewVariable", shape=(2, 0), datatype="s", unit=""
        )
        self.assertRaises(
            ValueError, data.initialize_info, "FooTitle", [independent], []
        )
